...
vtx::Game::Game(System& system) : system(system)
{
    reset();
}
...
void vtx::Game::update()
{
    if (board.white_to_play)
    {
        ...
        {
            int2 clicked_square = get_mouse_square();
...
float2 vtx::Game::mouse_ndc() const
{
    int2 board_position = get_board_position();
    int2 board_size = get_board_size();
    int2 current_position = system.window.mouse.position();

    int2 position_in_viewport = current_position - board_position;
    position_in_viewport.y = board_size.y - 1 - position_in_viewport.y;

    float2 ndc = (float2)position_in_viewport / (float2)board_size;
    ndc *= 2.0f;
    ndc -= float2(1.0f);
    return ndc;
}

int2 vtx::Game::get_board_position() const
{
    int2 viewport_size = system.viewport_size;
    int2 top_left = system.viewport_top_left;
...
int2 vtx::Game::get_board_size() const
{
    int2 viewport_position = system.viewport_top_left;
    int2 board_position = get_board_position();
    return system.viewport_size - (board_position - viewport_position) * 2;
}

int2 vtx::Game::get_mouse_square() const
{
    int2 viewport_position = system.viewport_top_left;
    int2 viewport_size = system.viewport_size;
    ...
    int2 clicked_square = (system.window.mouse.position() - board_position);
